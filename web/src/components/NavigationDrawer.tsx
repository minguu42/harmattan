...
export function NavigationDrawer() {
  return (
    <div className="min-h-svh w-280 border-r border-border bg-surface">
      <div className="flex h-56 items-center gap-8 px-12">
        ...
        <div className="text-sm font-semibold text-on-surface">ボッスン</div>
      </div>
      <ul className="mt-8">
...
function Indicator({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <li>
      <Link
        to="/"
        className="state-layer mx-8 flex h-36 items-center gap-8 rounded-lg px-8 text-sm text-on-surface"
      >
...
function ProjectIndicatorList() {
  ...
  return (
    <>
      <div className="mt-16 flex h-32 items-center justify-between pr-16 pl-12">
        <div className="text-sm font-semibold">プロジェクト</div>
        <ProjectCreateDialog />
      </div>
      <ul>
        {projects.map((p) => (
...
function ProjectCreatePopup({ closePopup }: { closePopup: () => void }) {
  ...
  return (
    <Dialog.Popup className="fixed top-1/2 left-1/2 -mt-32 w-400 -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-surface p-24">
      ...
      >
        <Field.Root>
          <Field.Control
            className="h-56 w-352 px-16 text-on-surface placeholder:text-on-surface-variant"
            required
            placeholder="プロジェクト名"
            value={name}
