...
- テストはすべて`_test`接尾辞の外部テストパッケージ（`package handler_test`など）に置く。
  - 非公開の関数や値をテストする場合は`export_test.go`（`package handler`など内部パッケージ）でエクスポートしてから外部テストパッケージで参照する。
- テーブルテストの構造体は`name string`フィールドでサブテスト名を持たせ、`t.Run(tt.name, ...)`で実行する。
  - 期待値がゼロ値（`nil`など）の場合はそのフィールドを記載しない。
- `t.Parallel()`の呼び出しの後は1行空ける。
